 ...
 #   for x in range(180, 254):
    ...
    if(len(cnts) > 0):
        ...
        if len(approx) == 4:
            ...
            #cv2.circle(image, (pts[0], pts[1]), 8, (0, 0, 255), -1) #red TL
            #cv2.circle(image, (pts[0]+ pts[2], pts[1]), 8, (0, 255, 0), -1) #green TR
            #cv2.circle(image, (pts[0], pts[1]+pts[3]), 8, (255, 0, 0), -1) #Blue BR
            #cv2.circle(image, (pts[0] + pts[2], pts[1]+ pts[3]), 8, (255, 255, 0), -1)#Cyan BL
            ...
            newRank = suit.copy()
            rankName = input("Rank: ")
            os.mkdir("pictures/test/{}".format(rankName))
            for x in range(0, 155):
                cv2.imwrite("pictures/test/{}/{}{}.jpg".format(rankName, rankName, x), imutils.rotate_bound(newRank.copy(), (random() * 360)))
            for r in ranks:
