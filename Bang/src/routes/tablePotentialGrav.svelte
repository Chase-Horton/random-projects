<script lang="ts">
	...
	class discoids {
        ...
        loadAscii(){
            if(DEVEL) console.log('loading ascii');
           //get next 8 cells from program and convert to binary string to insert into stack
            let ascii = '';
            for(let i = 1; i <= 8; i++){
                if(bangProgram[this.progIndex + i] == "!"){
                    ascii += '1';
                }else if(bangProgram[this.progIndex + i] == "?"){
                    ascii += '0';
                }
            }
            this.stack.push(Number(ascii));
            stack = [...this.stack];
            this.progIndex += 8;
        }
		outputTop() {
			if (DEVEL) console.log('outputting top');
			if (checkIfBinary(this.stack[this.stack.length - 1].toString())) {
				output.push(String.fromCharCode(parseInt(this.stack[this.stack.length - 1].toString(), 2)));
			} else {
				output.push(this.stack[this.stack.length - 1].toLocaleString("en-US"));
			}
			output = output;
		}
		outputStack() {
			...
			if (false) {
				...
				for (let char of this.stack) {
					if (checkIfBinary(char.toString())) {
						output.push(
							String.fromCharCode(parseInt(char.toString(), 2))
						);
					} else {
		...
		sumAB() {
			...
			let sum = this.stack[this.stack.length - 1] + this.stack[this.stack.length - 2];
			this.stack = this.stack.slice(0, this.stack.length - 2);
			this.stack.push(sum);
			stack = [...this.stack];
		}
		multAB() {
			if (DEVEL) console.log('multiplying a and b');
			let product = this.stack[this.stack.length - 1] * this.stack[this.stack.length - 2];
			this.stack = this.stack.slice(0, this.stack.length - 2);
			this.stack.push(product);
			stack = [...this.stack];
		}
		subAB() {
			if (DEVEL) console.log('subtracting a and b');
			let difference = this.stack[this.stack.length - 1] - this.stack[this.stack.length - 2];
			this.stack = this.stack.slice(0, this.stack.length - 2);
			this.stack.push(difference);
			stack = [...this.stack];
		}
		divAB() {
			if (DEVEL) console.log('dividing a and b');
			let quotient = this.stack[this.stack.length - 1] / this.stack[this.stack.length - 2];
			this.stack = this.stack.slice(0, this.stack.length - 2);
			this.stack.push(quotient);
			stack = [...this.stack];
		}
		startWhile() {
	...
	function checkIfBinary(x: string) {
		let isBinary = false;
	...
	function convertToBase(decimal: number, base: number) {
		...
		} else if (base == 10) outStr = convertDecToChars(decimal);
		else {
			outStr = out.join('');
		}
		return outStr;
	...
	function generateFunctionsStr(bangs: string, startIndex: number) {
		...
		for (let i = 0; i < bangList.length; i++) {
			if (bangList[i] == '!') {
                ...
                if (opDial[currentIndex].name == 'loadAscii') {
                    i+=8;
                }
			} else if (bangList[i] == '?') {
	...
	let INPUT = '!?!!?!?!?!?!?!';
	let programType;
	let bangProgram: string[] | (string | null);
	let binaryProgram;
	let decimalProgram;
	let base12Mprogram;
	let base10Program;
	...
	$: {
		...
		if (programType == 'bang') {
			...
			if (base == 10) base10Program = convertDecCharsToDec(base12Mprogram);
			else base10Program = convertBaseToDec(base, base12Mprogram);
			...
			if (base == 10) decimalProgram = convertDecCharsToDec(base12Mprogram);
			else decimalProgram = convertBaseToDec(base, base12Mprogram);
    ...
    let questions = [
		{ id: '!?!!?!!?!!?!!??!!!!?!!?!???!?!!!!??!??!?!?!!!??!!??!', text: `Calculate gravitational potential energy` },
		{ id: '!!!!', text: `Calculate Force` },
		{ id: '!?!?!', text: `What is another personal fact that an attacker could easily find with Google?` }
	];
...
<div class="pageContainer">
	<div style="display:flex">
		<div class="stripContainer">
			<TableWheel
				{test}
				bind:rotate={setOpRotation}
				bind:sliceNames={D.opDialNames}
				bind:active={activeCommand}
				bind:SLEEPTIME
			/>
